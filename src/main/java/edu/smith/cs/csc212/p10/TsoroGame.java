...
public class TsoroGame extends GFX {
	...
	/**
	 * Check when both players run out of their deisgnated 
	 * 3 tokens
	 * @return
	 */
	 ...
	 * 
	 * @param player -- either player 2 or player 2
	 * @return
	 */
	...
	static class TsoroCell {
		...
		/**
		 * Check if cell has a token (if it's empty)
		 * @return
		 */
	...
	public void update(double time) {
		...
		if (click != null && (this.state == TState.Player1Win ||
	...
	/**
	 * Makes moving token from one cell to another after players run 
			...
			if (cell.symbol != TMark.Empty && cell.contains(click)) {
				...
				if (this.state == TState.Player1Turn) {
					...
					if (cell.symbol == TMark.Player1) {
						// change it to empty (aka delete it)
					...
					// do the same as first case bu for plauer 2
